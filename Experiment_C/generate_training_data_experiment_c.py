#!/usr/bin/env python3
"""
festo_llama3.2_experiment_c — dataset generator  [TRUE MULTIVARIABLE MPC EDITION]
Extends Experiment A with a receding-horizon lookahead:
  • PREDICTION_HORIZON = 3 steps ahead simulated for candidate actions
  • Best action selected by minimising horizon cost (level error priority)
  • Predicted future states included in every user prompt
  • System prompt updated to teach horizon-aware reasoning
  • Same 4-field response schema preserved (schema reliability owned by adapter)

Targets: level 45% +- 5%, temp 35C +- 3C (level priority).
Output: 3,000 examples (2,750 normal + 250 anti-hallucination).
"""
...
OUT_FILE = Path(__file__).parent / "dataset_experiment_c.jsonl"
...
def short_reason(level, temp, pump, heater, valve, future_states=None):
    ...
    if future_states:
        l_converging = abs(future_states[-1][0] - TARGET_LEVEL) < abs(level - TARGET_LEVEL)
        t_converging = abs(future_states[-1][1] - TARGET_TEMP) < abs(temp - TARGET_TEMP)
        if l_converging and t_converging:
            h_part = "both converging"
        elif l_converging:
            h_part = "level converging, temp stable"
        elif t_converging:
            h_part = "temp converging, level stable"
        else:
            h_part = "horizon stable"
        return f"{l_part}; {t_part}; {h_part}"
...
def predict_horizon(level, temp, pump, valve, heater, inertia, steps=PREDICTION_HORIZON):
    future = []
...
def horizon_cost(future_states):
    cost = 0.0
    for i, (lv, tp) in enumerate(future_states):
        discount = 0.9 ** i
        cost += discount * (1.5 * abs(lv - TARGET_LEVEL) + 1.5 * abs(tp - TARGET_TEMP))
    return cost


def select_best_action(level, temp, prev_heater, inertia):
    pump_base, valve_base = compute_pump(level)
    heater_base = compute_heater(temp, prev_heater)
    candidates = []
    for p_delta in [0, -10, 10]:
        for v_flip in [False, True]:
            for h_flip in [False, True]:
                p = max(0, min(100, pump_base + p_delta))
                v = valve_base ^ v_flip
                h = heater_base ^ h_flip
                if p > 0 and v and level >= FLOAT_SWITCH_CUTOFF:
                    continue
                future = predict_horizon(level, temp, p, v, h, inertia)
                cost = horizon_cost(future)
                candidates.append((cost, p, v, h, future))
    candidates.sort(key=lambda x: x[0])
    best = candidates[0]
    return best[1], best[2], best[3], best[4]


def fmt_prediction(future_states):
    parts = []
    for i, (lv, tp) in enumerate(future_states, 1):
        parts.append(f"t+{i}: level={lv:.1f}%, temp={tp:.2f}C")
    return " | ".join(parts)


SYSTEM_PROMPT = (
    "You are a Festo water tank controller with predictive look-ahead. "
    "PRIMARY: keep level at 45% (range 40-50%). "
    "SECONDARY: keep temperature at 35C (range 32-38C). "
    "You are given the current sensor readings, recent history, AND a 3-step "
    "prediction of future states if the recommended action is applied. "
    "Use this prediction to reason about whether the action will converge to "
    "target over the horizon before outputting your decision. "
    "Think step by step: (1) assess current level error, (2) assess temp error, "
    "(3) check the predicted future states for BOTH level AND temperature convergence, "
    "then output ONLY a single JSON object with these exact keys: "
    "pump_power (integer 0-100), upper_valve_open (boolean true/false), "
    "heater_on (boolean true/false), reason (short string). "
    "NEVER use strings for boolean fields. NEVER add extra keys. "
    "Examples:\n"
    "Input: level=10%, temp=30C, predicted t+1: level=13.3%, temp=29.8C | t+2: level=16.6%, temp=30.0C | t+3: level=19.9%, temp=30.2C -> "
    "{\"pump_power\": 100, \"upper_valve_open\": false, \"heater_on\": true, "
    "\"reason\": \"level critical low, pump max; temp low, heater on; both converging\"}\n"
    "Input: level=44%, temp=35C, predicted t+1: level=44.1%, temp=35.0C | t+2: level=44.3%, temp=35.0C | t+3: level=44.5%, temp=35.0C -> "
    "{\"pump_power\": 10, \"upper_valve_open\": false, \"heater_on\": true, "
    "\"reason\": \"level slightly low; temp in band, heater on; level converging, temp stable\"}\n"
    "Input: level=60%, temp=40C, predicted t+1: level=58.4%, temp=39.8C | t+2: level=56.8%, temp=39.5C | t+3: level=55.3%, temp=39.3C -> "
    "{\"pump_power\": 0, \"upper_valve_open\": true, \"heater_on\": false, "
    "\"reason\": \"level high, draining; temp high, heater off; both converging\"}"
)


def build_user_msg(level, temp, history, future_states):
    return (
        f"Upper level: {level}%, Temperature: {temp}C, "
        f"Target level: {TARGET_LEVEL}%, Target temp: {TARGET_TEMP}C\n"
        f"History: {fmt_history(history)}\n"
        f"Predicted future (if recommended action applied): {fmt_prediction(future_states)}"
    )


# ---------- distribution: 8,500 normal records (same ratios, scaled to 9k total) ----------
raw = []
...
for level, temp, _ in raw:
    ...
    inertia = random.randint(0, THERMAL_INERTIA_STEPS)
    pump, valve, heater, future_states = select_best_action(level, temp, hist[-1]["heater"], inertia)
    response = {
        "pump_power": int(pump),
        "upper_valve_open": bool(valve),
        "heater_on": bool(heater),
        "reason": short_reason(level, temp, pump, heater, valve, future_states),
    }
    records.append({
        "messages": [
            {"role": "system", "content": SYSTEM_PROMPT},
            {"role": "user", "content": build_user_msg(level, temp, hist, future_states)},
            {"role": "assistant", "content": json.dumps(response)},
        ]
    })

# ---------- 250 anti-hallucination correction pairs ----------
ah_pool = [(l, t) for l in [10.0, 25.0, 40.0, 45.0, 47.0, 60.0, 80.0]
                 for t in [25.0, 32.0, 34.5, 35.0, 35.5, 38.0, 45.0]]
random.shuffle(ah_pool)
ah_scenarios = (ah_pool * (250 // len(ah_pool) + 1))[:250]
...
for level, temp in ah_scenarios:
    ...
    inertia = random.randint(0, THERMAL_INERTIA_STEPS)
    pump, valve, heater, future_states = select_best_action(level, temp, hist[-1]["heater"], inertia)
    user_msg = build_user_msg(level, temp, hist, future_states)
...
print(f"  Anti-hallucination: 250")
