...
exp05 (exp02 base) - Pure LLM Capability Test
...
# Set via env var, e.g. PROMPT_VARIANT=v2_system python -u llm_supervisor_exp05.py
...
# FILE PATHS
# =====================
#SENSOR_LOG_PATH    = "/scratch/rayarvid/exp05/festo_live/json_data.txt"
#LLM_CONTROL_PATH   = "/scratch/rayarvid/exp05/festo_live/llm_control.json"
# Summary + unified log are split per variant so multiple runs don't clobber each other.
#SUMMARY_STATS_PATH = f"/scratch/rayarvid/exp05/festo_live/llm_summary_stats_{PROMPT_VARIANT}.json"
#UNIFIED_LOG_PATH   = f"/scratch/rayarvid/exp05/festo_live/llm_unified_log_{PROMPT_VARIANT}.jsonl"
SENSOR_LOG_PATH    = "/scratch/rayarvid/Experiments/exp05/festo_live/json_data.txt"
LLM_CONTROL_PATH   = "/scratch/rayarvid/Experiments/exp05/festo_live/llm_control.json"
SUMMARY_STATS_PATH = f"/scratch/rayarvid/Experiments/exp05/festo_live/llm_summary_stats_{PROMPT_VARIANT}.json"
UNIFIED_LOG_PATH   = f"/scratch/rayarvid/Experiments/exp05/festo_live/llm_unified_log_{PROMPT_VARIANT}.jsonl"
...
ADAPTER_PATH = "/scratch/rayarvid/Experiments/exp05/festo_llama3.2_finetuned_exp05/checkpoint-1200"
...
# True hardware limits (matching exp05 training context)
MAX_SAFE_TEMP   = 38.0
MAX_SAFE_LEVEL  = 70.0
MAX_DECISIONS = 250
...
# LLM SETUP
# =====================
print("Loading fine-tuned Llama 3.2 exp05 model (exp02-based supervisor)...")
...
model.eval()
print("exp05 Model loaded - Pure LLM capability test")
...
# =====================
# PROMPT VARIANTS (one knob per row in the supervisor-comparison table)
# =====================
# v1_plain: NO system message. Everything (task, schema, targets) is in the
# user message. The bare-minimum baseline.
...
# v2_system: minimal SYSTEM message — role + schema + targets. No CoT cue,
# no examples. User message stays the bare sensor reading.
PROMPT_V2_SYSTEM = (
    "You are a Festo water tank controller. Output ONLY a single JSON object "
...
# v3_cot: v2 + Chain-of-Thought cue.
PROMPT_V3_COT = PROMPT_V2_SYSTEM + (
    " Think step by step about level error first, then temperature, before "
    "emitting the JSON."
)

# v4_fewshot: v3 + 3 few-shot examples (matches the original exp05 prompt).
...
SYSTEM_PROMPT = {
    "v1_plain":    None,                  # no system message at all
...
def main():
    ...
    print("=" * 70)
    print("LLM CONTROL AGENT exp05 (exp02-based) - PURE CAPABILITY TEST")
    print("=" * 70)
