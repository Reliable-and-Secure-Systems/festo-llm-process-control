#!/usr/bin/env python3
"""
festo_llama3.2_experiment_b — dataset generator  [PREDICTIVE CONTROL EDITION]
...
Output: 9,000 examples (8,500 normal + 500 anti-hallucination).
"""
...
OUT_FILE = Path(__file__).parent / "dataset_experiment_b.jsonl"
...
def horizon_cost(future_states):
    ...
    for i, (lv, tp) in enumerate(future_states):
        discount = 0.9 ** i
        cost += discount * (2.0 * abs(lv - TARGET_LEVEL) + abs(tp - TARGET_TEMP))
    return cost
...
SYSTEM_PROMPT = (
    ...
    "(3) check the predicted future states to confirm the action is converging, "
    ...
    "Examples:\n"
    "Input: level=10%, temp=30C, predicted t+1=13.3%, t+2=16.6%, t+3=19.9% -> "
    "{\"pump_power\": 100, \"upper_valve_open\": false, \"heater_on\": true, "
    "\"reason\": \"level critical low, pump max; temp low, heater on; horizon converging\"}\n"
    "Input: level=44%, temp=35C, predicted t+1=44.1%, t+2=44.3%, t+3=44.5% -> "
    "{\"pump_power\": 10, \"upper_valve_open\": false, \"heater_on\": true, "
    "\"reason\": \"level slightly low; temp in band, heater on; horizon stable\"}\n"
    "Input: level=60%, temp=40C, predicted t+1=58.4%, t+2=56.8%, t+3=55.3% -> "
    "{\"pump_power\": 0, \"upper_valve_open\": true, \"heater_on\": false, "
    "\"reason\": \"level high, draining; temp high, heater off; horizon draining toward target\"}"
)
