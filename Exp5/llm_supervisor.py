...
exp08 (exp07 base) - True Multivariable MPC: Equal Level+Temp Horizon Optimization
...
# FILE PATHS
# =====================
#SENSOR_LOG_PATH    = "/scratch/rayarvid/Experiments/exp08/festo_live/json_data.txt"
#LLM_CONTROL_PATH   = "/scratch/rayarvid/Experiments/exp08/festo_live/llm_control.json"
# Summary + unified log are split per variant so multiple runs don't clobber each other.
#SUMMARY_STATS_PATH = f"/scratch/rayarvid/Experiments/exp08/festo_live/llm_summary_stats_{PROMPT_VARIANT}.json"
#UNIFIED_LOG_PATH   = f"/scratch/rayarvid/Experiments/exp08/festo_live/llm_unified_log_{PROMPT_VARIANT}.jsonl"

SENSOR_LOG_PATH    = "/scratch/rayarvid/Experiments/Experiment_3_equal_cycles/festo_live/json_data.txt"
LLM_CONTROL_PATH   = "/scratch/rayarvid/Experiments/Experiment_3_equal_cycles/festo_live/llm_control.json"
SUMMARY_STATS_PATH = f"/scratch/rayarvid/Experiments/Experiment_3_equal_cycles/festo_live/llm_summary_stats_{PROMPT_VARIANT}.json"
UNIFIED_LOG_PATH   = f"/scratch/rayarvid/Experiments/Experiment_3_equal_cycles/festo_live/llm_unified_log_{PROMPT_VARIANT}.jsonl"
...
_adapter_dir = "/scratch/rayarvid/Experiments/exp08/festo_llama3.2_finetuned_exp08"
_checkpoints = sorted(
...
# LLM SETUP
# =====================
print(f"Loading fine-tuned Llama 3.2 exp08 model...")
...
model.eval()
print("exp08 Model loaded - True Multivariable MPC test")
...
PROMPT_V4_FEWSHOT = PROMPT_V3_COT + (
    " Examples:\n"
    'Input: level=10%, temp=30C, predicted t+1: level=13.3%, temp=29.8C | t+2: level=16.6%, temp=30.0C | t+3: level=19.9%, temp=30.2C -> {"pump_power": 100, "upper_valve_open": false, "heater_on": true, "reason": "level critical low, pump max; temp low, heater on; both converging"}\n'
    'Input: level=44%, temp=35C, predicted t+1: level=44.1%, temp=35.0C | t+2: level=44.3%, temp=35.0C | t+3: level=44.5%, temp=35.0C -> {"pump_power": 10, "upper_valve_open": false, "heater_on": true, "reason": "level slightly low; temp in band, heater on; level converging, temp stable"}\n'
    'Input: level=60%, temp=40C, predicted t+1: level=58.4%, temp=39.8C | t+2: level=56.8%, temp=39.5C | t+3: level=55.3%, temp=39.3C -> {"pump_power": 0, "upper_valve_open": true, "heater_on": false, "reason": "level high, draining; temp high, heater off; both converging"}'
)
...
def build_prompt(data, recent_history):
    ...
    # Predictive look-ahead — multivariable MPC (exp08: equal level+temp weight)
...
def main():
    ...
    print("=" * 70)
    print("LLM CONTROL AGENT exp08 - TRUE MULTIVARIABLE MPC TEST")
    print("=" * 70)
